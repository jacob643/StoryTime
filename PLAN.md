...
- [ ] **3.6.24** Diversify continuation prompts — replace the single `OUTCOME_DIRECTIONS` string per tier with a list of ~10 alternative phrasings per tier in `prompt_engine.py`. `build_prompt` picks a random entry from the list for each call so the LLM gets varied direction instructions instead of the same wording every time. Add test that verifies all tiers have >= 10 entries and that randomness works.
- [ ] **3.6.25** Auto-scroll text display when paragraph exceeds viewport — in `frontend/script.js`, after new text is set in `#textDisplay`, call `textDisplayContainer.scrollTop = textDisplayContainer.scrollHeight` to scroll to the bottom so the user always sees the current typing area. Only scroll if the container is already scrolled near the bottom (to avoid stealing scroll position if the user is reading earlier text).
- [ ] **3.6.26** Redesign simulate for single-paragraph testing — change simulate so it generates one paragraph at the given CPM, appends it to history, then the user can continue typing manually instead of being locked into simulated auto-advance. This means splitting simulate into two modes: `/api/simulate` generates a paragraph and stops (no auto-loop), and the frontend `window.simulate(cpm, dev)` calls it once, displays the result, and lets the user type the next paragraph naturally. May need a way to seed rolling_splits for testing.
- [ ] **3.6.27** Collapse multi-paragraph LLM output into single paragraph — if the LLM returns text with newlines/blank lines between paragraphs, collapse them into a single paragraph with single spaces between sentences. Also normalize whitespace: ensure exactly one space after `.`, `!`, `?` and no double spaces anywhere. Handle this in `prompt_engine.py`'s `parse_llm_response` or a new `normalize_whitespace()` function applied after `parse_llm_response` in all route handlers.
- [ ] **3.6.28** Investigate tier outcome mismatch with simulate — the user had rolling=[500x24] and split_speeds=[505x14] but got tier=2 instead of tier=4. Root cause: double `min_stddev_cpm` floor in `compute_outcome_tier` (line 93 of `game_logic.py`) that defeated σ/√N. Fixed in 3723a42. Add a regression test with the exact reproduce case.

...
- [x] **3.6.9** Simulate shouldn't restart the story — if `sessionId` is already set in the frontend, `sendSimulate()` no longer resets it; it just sets `simulatedCpm`/`simulatedDeviation` and shows a message. Only creates a new session on first call.
